...
class DynamicInventoryReport(models.Model):
    ...
    def _get_report_sub_lines(self, data, report, date_from, date_to):
        ...
        if data.get('report_type') == 'report_by_transfers':
            ...
            query = """
                 select 
                    product_product.standard_price as value_float,
            ...
            term = ' Where '
            if data.get('date_from'):
                query += " Where stock_quant.create_date >= '%s' " % data.get(
                    'date_from')
                term = ' AND '
            if data.get('date_to'):
                query += term + " stock_quant.create_date <= '%s' " % data.get(
                    'date_to')
            self._cr.execute(query)
    ...
    def get_inventory_xlsx_report(self, data, response, report_data, dfr_data):
        ...
        if filters.get('report_type') == 'report_by_categories':
            ...
            for rec_data in report_data_main:
                ...
                if rec_data['value_float']:
                    value = list(rec_data['value_float'].values())[0]
                    sheet.write(row, col + 3, value, txt_l)
